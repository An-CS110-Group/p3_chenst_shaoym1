    ...
    for (y = 0; y < a.dimY; y++) {
        for (x = 0; x < a.dimX; x++) {
            ...
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[0] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[0] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[0] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] / gv.sum[ext - deta] * (float) get_pixel(a, x - ext + i, y)[0]; }
            pc[0] = (float) sum;
            ...
            sum = 0;
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[1] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[1] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[1] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] / gv.sum[ext - deta] * (float) get_pixel(a, x - ext + i, y)[1]; }
            pc[1] = (float) sum;
            ...
            sum = 0;
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[2] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[2] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x - ext + i, y)[2] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] / gv.sum[ext - deta] * (float) get_pixel(a, x - ext + i, y)[2]; }
            pc[2] = (float) sum;
        }
    }
...
#pragma omp parallel for schedule(dynamic) default(none) private(y) shared(a, x, b, pc, gv, sum, ext, deta, i)
    for (y = 0; y < a.dimY; y++) {
        for (x = 0; x < a.dimX; x++) {
            ...
            sum = 0;
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[0] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[0] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[0] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[0] / gv.sum[ext - deta]; }
            pc[0] = (float) sum;
            ...
            sum = 0;
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[1] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[1] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[1] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[1] / gv.sum[ext - deta]; }
            pc[1] = (float) sum;
            ...
            sum = 0;
            for (i = deta; i < gv.length - deta - 4;) {
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[2] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[2] / gv.sum[ext - deta];
                ++i;
                sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[2] / gv.sum[ext - deta];
            ...
            for (; i < gv.length - deta; ++i) { sum += gv.data[i] * (float) get_pixel(a, x, y - ext + i)[2] / gv.sum[ext - deta]; }
            pc[2] = (float) sum;
        }
    }
    return b;
