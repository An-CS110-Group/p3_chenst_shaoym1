...
Image gb_h(Image a, FVec gv, float *gvData) {
    ...
    float *pixels = aligned_alloc(256, (((3 * (a.dimX + 2 * ext + 1) * a.dimY * sizeof(float)) >> 9) + 1) << 9);

#pragma omp parallel for schedule(static) default(none) shared(a, ext, pixels)
...
Image apply_gb(Image a, FVec gv) {
    __attribute__((aligned(256))) float gvData[3 * gv.length + 10];
